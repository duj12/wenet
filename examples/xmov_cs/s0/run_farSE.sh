...
train_set=farfield/train_farA1A2_SE3
dev_set=farfield/valid_farA2_SE3

test_sets="farfield/thchs_farA1_SE3 farfield/thchs_farA2_SE3  farfield/valid_farB2_SE3 \
 farfield/valid_farA1_SE3 farfield/valid_farA2_SE3 farfield/thchs_farB2_SE3 "
  ...
  for x in ${dev_set} ${train_set} ; do
    if [ $data_type == "shard" ]; then
      tools/make_shard_list.py --num_utts_per_shard $num_utts_per_shard \
        --num_threads 32 data/$x/wav.scp \
        data/$x/text \
        $(realpath data/$x/shards) \
        data/$x/data.list
    else
      tools/make_raw_list.py data/$x/wav.scp \
      data/$x/text \
      data/$x/data.list
    fi
  done
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  ...
  if [ ! -z $nnlm_file ] ; then
    py_decode_opts="--lm_file $nnlm_file --lm_firstpass_weight $lm_firstpass_weight --lm_secondpass_weight $lm_secondpass_weight "
    py_decode_suffix="_nnlm_1st${lm_firstpass_weight}_2nd${lm_secondpass_weight}"
  else
    py_decode_opts=
    py_decode_suffix=
  fi
  ...
  {
    test_dir="$dir/test_${mode}${decoding_chunk_size:+_chunk$decoding_chunk_size}${py_decode_suffix}/${test}"
    mkdir -p $test_dir
    ...
    python wenet/bin/recognize.py --gpu $gpu_id \
      ...
      --bpe_model ${bpecode} $py_decode_opts \
...
#      data/${test}/text $test_dir/text > $test_dir/cer
