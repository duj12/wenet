...
class WenetModel(object):
    def __init__(self, model_config, device):
        ...
        self.tlg_dir = params.get("tlg_dir", "")
        self.tlg_decoding_config = params.get("tlg_decoding_config", "wfst_decoding_config.yaml")
        with open(self.tlg_decoding_config, 'rb') as f:
            self.tlg_decoding_config = yaml.load(f, Loader=yaml.Loader)
        print("WFST decoder config:\n", self.tlg_decoding_config)
        self.decoding_method = params.get("decoding_method", "tlg")
        if "tlg" in self.decoding_method and os.path.exists(self.tlg_dir+"/TLG.fst"):
            print(f"loading {self.tlg_dir}/TLG.fst")
            self.decoder = RivaWFSTDecoder(len(self.vocab),
                                           self.tlg_dir,
                                           self.tlg_decoding_config,
                                           self.beam_size)
